...
class Item extends \OpenTHC\POS\Controller\API\Base
{
	function __invoke($REQ, $RES, $ARG)
	{
		...
		if (empty($source_data['inventory']['id'])) {
			return $this->failure($RES, 'Invalid Inventory [ABI-024]', 400);
		}
		...
		// Check Inventory
		$inv = new \OpenTHC\POS\Inventory($dbc, $source_data['inventory']['id']);
		if (empty($inv['id'])) {
			return $this->failure($RES, 'Invalid Inventory [ABI-038]', 400);
		}

		if (empty($source_data['unit_price'])) {
			$source_data['unit_price'] = $inv['sell'];
		}

		if ($inv['qty'] < $source_data['unit_count']) {
